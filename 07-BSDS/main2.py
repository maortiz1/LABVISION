...
#for i in filenames:
i = "100075.jpg"
#os.mkdir(os.path.join('BSR','gmmlab'))

for filenam in filenames[131::-1]:
  ...
  if ext=='.jpg':
     ...
     count = 0
     for j in K:   
        seg = segmentByClustering(rgbImage=temp, colorSpace='lab', clusteringMethod='gmm', numberOfClusters=j)
        #ipdb.set_trace()   
        concat[0,count] = seg
        count = count+1
        print('K=%d , method=gmmlab'%(j))
     filenam2,ext =os.path.splitext(filenam)  
     
     sio.savemat(os.path.join('BSR','gmmlab','%s.mat'%(filenam2)), {'segs':concat})
